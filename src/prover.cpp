    ...
    for (int i = 1; i < C.size; ++i)
    {
        ...
        for (u64 g = 0; g < C.circuit[i].size; ++g)
        {
            ...
            switch (ty) {
                ...
                break;
                case gateType::Zero:
                    circuitValue[i][g] = F_ZERO;
                break;
                case gateType::Input:
                    circuitValue[i][g] = F((i64) u);
                break;
                case gateType::Relay:
                    circuitValue[i][g] = circuitValue[i - 1][u];
                break;
                case gateType::Not:
                    circuitValue[i][g] = F::one() - circuitValue[i - 1][u];
                break;
                case gateType::Minus:
                    circuitValue[i][g] = circuitValue[i - 1][u] - circuitValue[l][v];
                break;
                case gateType::AntiMinus:
                    circuitValue[i][g] = circuitValue[l][v] - circuitValue[i - 1][u];
                break;
                case gateType::Xor:
                ...
                case gateType::Naab:
                    x = &circuitValue[i - 1][u];
                    y = &circuitValue[l][v];
                    circuitValue[i][g] = *y - (*x) * (*y);
                break;
                case gateType::AntiNaab:
                    x = &circuitValue[i - 1][u];
                    y = &circuitValue[l][v];
                    circuitValue[i][g] = *x - (*x) * (*y);
                break;
                default:
                    assert(false);
            }
...
void prover::init()
{
    ...
    r_u.resize(max_bl);
    r_liu.resize(max_bl);
    for (int i = 1; i < C.size; ++i) r_v[i].resize(C.circuit[i].maxDadBitLength);
 ...
 */
void prover::sumcheckInitPhase1()
{
    F::isCounting = true;
    F::isSumchecking = true;
    ...
    for (u64 g = 0; g < cur.size; ++g)
    {
        ...
        switch (ty) {
            case gateType::Add:
                tmp_add[u] = tmp_add[u] + circuitValue[l][cur.dadId[l][v]] * tmp;
                tmp_mult[u] = tmp_mult[u] + tmp;
                break;
            case gateType::Mult:
                tmp_mult[u] = tmp_mult[u] + circuitValue[l][cur.dadId[l][v]] * beta_g[g];
                break;
            case gateType::Not:
            ...
            case gateType::Relay:
                ...
                break;
            case gateType::Minus:
                tmp_add[u].b = tmp_add[u].b - circuitValue[l][cur.dadId[l][v]] * tmp;
                tmp_mult[u] = tmp_mult[u] + tmp;
                break;
            case gateType::AntiMinus:
                tmp_add.at(u) = tmp_add.at(u) + circuitValue[l].at(cur.dadId[l][v]) * tmp;
                tmp_mult[u].b = tmp_mult[u].b - tmp;
                break;
            case gateType::Xor:
                tmp_add[u] = tmp_add[u] + tmp * circuitValue[l][cur.dadId[l][v]];
                tmp_mult[u] = tmp_mult[u] + (tmp - (F(2) * circuitValue[l][cur.dadId[l][v]] * tmp));
                break;
            case gateType::Naab:
                tmp_add[u] = tmp_add[u] + tmp * circuitValue[l][cur.dadId[l][v]];
                tmp_mult[u].b = tmp_mult[u].b - (circuitValue[l][cur.dadId[l][v]] * tmp);
                break;
            case gateType::AntiNaab:
                tmp_mult[u] = tmp_mult[u] + (tmp - (circuitValue[l][cur.dadId[l][v]] * tmp));
                break;
            case gateType::Zero: break;
            default:
                assert(false);
        }
    }
...
void prover::sumcheckInitPhase2()
{
    ...
    int max_bit_length = cur.maxDadBitLength;
    ...
    for (u64 g = 0; g < cur.size; ++g)
    {
        ...
        switch (ty) {
            ...
            case gateType::Mult:
                ...
                break;
            case gateType::Not:
                addVArray[l][0] = addVArray[l][0] + (tmp - tmp * V_u);
                break;
            case gateType::Minus:
                addVArray[l][v] = addVArray[l][v] + tmp * V_u;
            ...
            case gateType::Xor:
                addVArray[l][v] = addVArray[l][v] + tmp * V_u;
                multArray[l][v] = multArray[l][v] + (tmp - (F(2) * V_u * tmp));
                break;
            case gateType::Naab:
...
quadratic_poly prover::sumcheckUpdate(const F &previousRandom, vector<F> &r_arr, int n_pre_layer) {
    F::isCounting = true;
    F::isSumchecking = true;
    prove_timer.start();

    if (round) r_arr.at(round - 1) = previousRandom;
    ++round;
    quadratic_poly ret(F_ZERO, F_ZERO, F_ZERO);

    add_term = add_term == F_ZERO ? F_ZERO : add_term * (F_ONE - previousRandom);
    for (int i = 0; i < n_pre_layer; ++i)
        ret = ret + sumcheckUpdateEach(previousRandom, i);
...
quadratic_poly prover::sumcheckUpdateEach(const F &previousRandom, int idx) {
    auto &tmp_v = Vmult[idx];
    auto &tmp_add = addVArray[idx];
    auto &tmp_mult = multArray[idx];

    if (total[idx] == 1) {
        tmp_v[0] = tmp_v[0].eval(previousRandom);
        tmp_add[0] = tmp_add[0].eval(previousRandom);
        tmp_mult[0] = tmp_mult[0].eval(previousRandom);
    ...
    for (u64 i = 0; i < (total[idx] >> 1); ++i) {
        ...
        tmp_v[i] = interpolate(tmp_v[g0].eval(previousRandom), tmp_v[g1].eval(previousRandom));
        tmp_add[i] = interpolate(tmp_add[g0].eval(previousRandom), tmp_add[g1].eval(previousRandom));
        tmp_mult[i] = interpolate(tmp_mult[g0].eval(previousRandom), tmp_mult[g1].eval(previousRandom));
...
{
    prove_timer.start();
    r_v[sumcheckLayerId][round - 1] = previousRandom;
    for (int i = 0; i < sumcheckLayerId; ++i) {
...
void prover::sumcheckLiuFinalize(const F &previousRandom, F &claim) {
    prove_timer.start();
    r_liu[round - 1] = previousRandom;
    claim = total[0] ? Vmult[0][0].eval(previousRandom) : Vmult[0][0].b;
...
hyrax_p224::polyProver &prover::commitInput(const vector<G> &gens) {
    if (C.circuit[0].size != (1ULL << C.circuit[0].bitLength))
        circuitValue[0].resize(1ULL << C.circuit[0].bitLength, F_ZERO);
    poly_p = std::make_unique<hyrax_p224::polyProver>(circuitValue[0], gens);
    return *poly_p;
}
#endif