...
void verifier::betaInitPhase1(int depth) {
...
void verifier::predicatePhase1(int layer_id) {
    ...
    coeff_l[(u64) gateType::Relay] = F_ZERO;
    ...
    fill(coeff_r[(u64) gateType::Minus].begin(), coeff_r[(u64) gateType::Minus].end(), F_ZERO);
    fill(coeff_r[(u64) gateType::AntiMinus].begin(), coeff_r[(u64) gateType::AntiMinus].end(), F_ZERO);
    ...
    fill(coeff_r[(u64) gateType::Mult].begin(), coeff_r[(u64) gateType::Mult].end(), F_ZERO);
...
void verifier::predicatePhase2(int layer_id) {
    ...
    coeff_l[(u64) gateType::Relay] *= beta_v[0];
    for (u64 g = 0; g < cur.size; ++g) {
        auto &gate = cur.gates[g];
        switch (gate.ty) {
            case gateType::Add:
...
F verifier::getFinalValue(int layer_id, const F &claim_u,
    ...
    auto res = coeff_l[(u64) gateType::Not] * (F_ONE - claim_u)
               + coeff_l[(u64) gateType::Relay] * claim_u;
    for (int j = 0; j < layer_id; ++j)
    {
        auto tmp = coeff_r[(u64) gateType::Add][j] * (claim_u + claim_v[j])
                   + coeff_r[(u64) gateType::Mult][j] * (claim_u * claim_v[j])
                   + coeff_r[(u64) gateType::Minus][j] * (claim_u - claim_v[j])
                   + coeff_r[(u64) gateType::AntiMinus][j] * (claim_v[j] - claim_u)
...
bool verifier::verify()
{
    ...
    u8 logn = C.circuit[0].bitLength;
    u64 n_sqrt = 1ULL << (logn - (logn >> 1));
    vector<G> gens(n_sqrt);
    for (auto &x: gens) x = G::random();

    poly_v = std::make_unique<hyrax_p224::polyVerifier>(p -> commitInput(gens), gens);
#endif
...
#ifdef USE_HYRAX_P224
	r_liu.resize(C.circuit[0].bitLength);
    if (!poly_v -> verify(r_liu, previousSum)) {
...
bool verifier::verifyPoly(const virgo::__hhash_digest &merkle_root_l, const F &previousSum) {
    ...
    F input_0;
    auto mask = std::vector<F>(1024, F_ZERO);
